...
**Note:** 
1. name in `__init__.py` or `setup.py` doesn't mind `-` or `_`, only "package folder name" would matter (because folder name will be used as library name, if you `import lambdata-nov05`, Python `import` can't read `-`, if you `import lambdata_nov05`, Python would throw error `ModuleNotFoundError: No module named 'lambdata_nov05'`).  
